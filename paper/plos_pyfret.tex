...
\begin{figure}
   \begin{center}
      ...
      \caption{RASP analysis of FRET data from a mixture of a high-FRET and low-FRET duplex. The black line shows the baseline peak areas for the two duplexes. Other lines show the FRET histograms generated from bursts that occurred at the indicated time following a high-FRET burst. Note the greatly increased peak area for high-FRET bursts, demonstrating the recurrence of high-FRET events as molecules diffuse back into the confocal volume.}
      \label{fig:fig6_plots}
   \end{center}
\end{figure}
...
\begin{figure}
   \begin{center}
      ...
      \caption{Evaluating the effect of detection window and bin time on the DCBS burst search algorithm for FRET data. A) Varying the length of the time-bin has little effect on the performance of the algorithm. B) Reducing the length of the detection window selects for very bright bursts. This reduces the number of detected bursts but does not affect the calculated FRET efficiency.}
      \label{fig:fig7_plots}
   \end{center}
\end{figure}
